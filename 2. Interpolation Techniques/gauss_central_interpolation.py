...
def gauss_central_interpolation(x, y, target_x):
    n = len(x)
    h = x[1] - x[0]
    ...
    # Calculate the result using Gauss's central difference formula
    result = diff[0][0]
    for j in range(1, n):
        factor = 1.0
        for k in range(j):
            factor *= (u - k) if k % 2 == 0 else (u + k + 1)
        term = diff[j][j] * factor / (j * (j + 1))
        result += term
...
def main():
    try:
        ...
        result = gauss_central_interpolation(x, y, target_x)
