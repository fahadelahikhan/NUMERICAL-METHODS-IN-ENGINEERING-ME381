...
def newton_forward_interpolation(x, y, target_x, show_table=False):
    """
    ...
    # p = (target_x - x_0) / h where x_0 is the first point
    p = (target_x - x[0]) / h

    # Start with the first y value
    result = diff[0][0]

    # Add terms: p*Δy_0/1! + p(p-1)*Δ²y_0/2! + ...
    for j in range(1, n):
        term = diff[0][j]
...
def main():
    ...
    try:
        ...
        result = newton_forward_interpolation(x, y, target_x, show_table)
